...
def test_treading_cost(N=10**4, repeats=10, work=1):
    ...
    phyiscal_cores = np.arange(
        1, psutil.cpu_count(logical=False)).astype(np.int32)
...
if __name__ == "__main__":
    ...
    for n_work, work in enumerate([1, 10]):
        ...
        for n, N in enumerate([10**3, 10**4, 10**5, 10**6]):
            ...
            ax.plot(phyiscal_cores, t0_out[1]/t1_out[:, 1], c=colours[n],
                    label=f'N={N:1.0e}, work={work:3d}', linestyle=linestyle[n_work])
