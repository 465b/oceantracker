...
#-----------------------------------------------    
run_name = '22_11_01_depth_losses_v46'
#-----------------------------------------------
...
for stickiness in np.linspace(0, 1, 5):
    for initial_size_list in [5e-6, 1e-5, 5e-5]:
...
for light_limitation_threshold in [15, 30, 45]:
    df = pd.concat([df, pd.DataFrame({'initial_size': 5e-6,
                                 'stickiness': 1,
...
for light_limited_culling_prop in [3.56e-05/2, 3.56e-05, 3.56e-05+3.56e-05/2]:
    df = pd.concat([df, pd.DataFrame({'initial_size': 5e-6,
                                 'stickiness': 1,
                                 'light_limitation_threshold': 30,
...
for index, case in df.iterrows():
        ...
        "trajectory_modifiers": {
            "light_starvation_induced_mortality": {
                ...
                "cull_interval": 60,
                "cull_status_greater_than": "dead",
                "required_illumination": case['light_limitation_threshold'], #W/m2
                "probability_of_culling": case['light_limited_culling_prop'],
            },
        },
        "particle_properties": {
                "radius_spherical": {
                    "class_name": "oceantracker.particle_properties.buoyancy.RadiusSpherical",
                    "initial_value": case['initial_size']
                },
                "density_spherical": {
                ...
                "collision_very_fine_silt": {
                    ...
                    "radius": "radius_spherical",
                    "initial_radius": case['initial_size'],
                    "stickyness": case['stickiness'],
