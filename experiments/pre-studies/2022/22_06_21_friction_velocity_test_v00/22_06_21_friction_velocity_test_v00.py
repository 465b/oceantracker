...
#-----------------------------------------------
run_name = '22_06_21_friction_velocity_test_v00'
#-----------------------------------------------
...
splitting_ratios = [1.875e-04] #np.linspace(0,0.00025,sa_resolution)
		...
		"particle_release_groups": [
			{
				...
				"pulse_size": 10000, #int(max_particle/2),
				"z_min": -1,
				"z_max": 1,
				"release_interval": 0
			}
...
for sinking in sinking_parameters:
    for splitting in splitting_ratios:
        for friction_velocity in np.linspace(0.005,0.015,5):
            diel_cases.append({
                "user_velocity_modifiers": [
                    {
                        "class_name": "oceantracker.particle_velocity.terminal_velocity.AddDielVelocity",
                        "mean": sinking
                    }
                ],
                "user_trajectory_modifiers": [
                    {
                        ...
                        "critical_friction_velocity": friction_velocity
                    },
                    {
                        "class_name": "oceantracker.user_trajectory_modifiers.split_particles.ParticleSplit",
                        "splitting_interval": 100,
...
runInfo = ocean_tracker_main.run_oceantracker(params)
runInfo_file_name  = runInfo[0]
print(runInfo_file_name)

#runInfo_file_name = '/scratch/local1/output/22_06_20_LDFR_reproduction_test_v00/22_06_20_LDFR_reproduction_test_v00_runInfo.json'
for ii,velocity in enumerate(cases):
    case = load_output_files.get_case_info_file_name(runInfo_file_name,ncase = ii)
    particle_plot.animate_particles(case,movie_file=run_name+str(ii)+'.mp4')