...
class depth_losses(ParticleProperty):

    def __init__(self):
        super().__init__()
        self.add_default_params({'name': PVC('depth_losses', str) ,
                                 'description': PVC( ' light starved plankton', str),
                                 'initial_value': PVC(None, float),
                                 'max_time_wo_light': PVC(3600*24*1., float),
                                 'depth_threshold': PVC(1,int)})
    ...
    def update(self,active):
        ...
        suffocating = np.where(particle_depth > self.params['depth_threshold'])
